...
import { PickingManager, SelectModelEvent, DeselectModelEvent } from "../PickingManager";
import {
...
export class TransformInspector {
  private static instance: TransformInspector

  position: Vector3UIControl;
  rotation: Vector3UIControl;
  scale: Vector3UIControl;
  gizmoSelect: GizmoSelectionMode;
  constructor(gizmoManager: any) {
    ...
    this.gizmoSelect = new GizmoSelectionMode(gizmoManager);

    TransformInspector.instance = this
  }
  ...
  UpdatePickedPosition(control: any) {
    const transformUI = TransformInspector.instance;
    const pickedObject = PickingManager.GetSelectedObject();
  ...
  UpdatePickedRotation(control: any) {
    ...
    if (control._isFocused && pickedObject) {
      ...
      switch (control._managedInput) {
        case "x":
          pickedObject.rotation.x = MathUtilities.DegreesToRadians(
            parseFloat(rotUI.x.text)
          );
          break;
        case "y":
          pickedObject.rotation.y = MathUtilities.DegreesToRadians(
            parseFloat(rotUI.y.text)
          );
          break;
        case "z":
          pickedObject.rotation.z = MathUtilities.DegreesToRadians(
  ...
  UpdateTransformValues() {
    const transformUI = TransformInspector.instance;
    const picked = PickingManager.GetSelectedObject();
    if (!picked) return;
    ...
    if (!rotUI.x._isFocused)
      rotUI.x.text = MathUtilities.RadiansToDegrees(picked.rotation.x).toFixed(
        0
      );
    if (!rotUI.y._isFocused)
      rotUI.y.text = MathUtilities.RadiansToDegrees(picked.rotation.y).toFixed(
        0
      );
    if (!rotUI.z._isFocused)
      rotUI.z.text = MathUtilities.RadiansToDegrees(picked.rotation.z).toFixed(
        0
      );
 ...
 */
export class GizmoSelectionMode {
  static instance: GizmoSelectionMode;
  gizmoManager: any;
  ...
  constructor(gizmoManager: any) {
    ...
    GizmoSelectionMode.instance = this;
  }
  ...
  SetGizmoByKeyboardInput(keyInfo: any) {
    const gizmoSelect = GizmoSelectionMode.instance;
  ...
  AttachNewMesh(evnt: SelectModelEvent) {
    GizmoSelectionMode.instance.gizmoManager.attachToMesh(evnt.selectedModel);
  }

  DetachMesh(evnt: DeselectModelEvent) {
    GizmoSelectionMode.instance.gizmoManager.attachToMesh(null);
  }
