  ...
  EnableDragMoveBehaviour(moveController: any) {
    ...
    moveController.onPointerDownObservable.add(
      () => {
        ...
        GUI_MenuManager.instance.AddCallback(func, menu._callbackID);
      }
    );

    moveController.onPointerUpObservable.add(
      () => {
        GUI_MenuManager.instance.RemoveCallback(menu._callbackID);
      }
    );
  }
}
...
export class GUI_MenuToggle {
   ...
   * @param {*} linkedMenu  - The GUI_Menu instance it toggles on or off.
   */
  constructor(control: Control, linkedMenu: GUI_Menu) {
    ...
    this.linkedMenu = linkedMenu; //Reference to GUI_Menu instance to disable/enable by this toggle.

    this.activeColor = ColorScheme.Selected;
    this.inactiveColor = ColorScheme.Selectable_Transparency;
  ...
  Disable() {
    ...
    this.control.background = this.inactiveColor;
    this.control.alpha = .7
...
*/
export class GUI_ToggleGroup {
  name: string;
  id: number;
  menuToggles: GUI_MenuToggle[];
  /**
   * Constructor.
   * @class
   ...
   * @param {GUI_MenuToggle} toggle - Toggle instance to add into this toggle group
   */
  AddToggle(toggle: GUI_MenuToggle) {
    if (!(toggle instanceof GUI_MenuToggle)) {
   ...
   * @param {GUI_MenuToggle} toggle - toggle instance to activate
   * @param {bool} activeState
   */
  ActivateToggle(toggle: GUI_MenuToggle, activeState?: boolean) {
 ...
 */
export class GUI_MenuManager {
  static instance: GUI_MenuManager;
  toggleGroups: GUI_ToggleGroup[];
  callbacks: any[];
  ...
  ) : any
  {
    ...
    const guiMenu = new GUI_Menu(container);
    ...
    const guiToggle = new GUI_MenuToggle(toggleButton, guiMenu);
  ...
  FindOrCreateToggleGroup(name: string) {
    ...
    if (!group)
    {
      group = new GUI_ToggleGroup(name);
