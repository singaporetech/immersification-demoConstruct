import { DC_EventData, DC_EventListener } from "./EventListener";

export interface DC_GlobalEventData extends DC_EventData{
    Key() : symbol
}

export class DC_GlobalEvents{
    private static map: Map<Symbol, DC_EventListener<any>> = new Map()

    static ContainsListener(symbol: Symbol){
        return DC_GlobalEvents.map.has(symbol)
    }

    static RegisterListener<T extends DC_GlobalEventData>(symbol: Symbol, listener: DC_EventListener<T>){
    ...
    static AddCallback<T extends DC_GlobalEventData>(symbol: Symbol, callback: (data: T)=>void){
        ...
        if(listener === undefined){
            listener = new DC_EventListener<T>()
