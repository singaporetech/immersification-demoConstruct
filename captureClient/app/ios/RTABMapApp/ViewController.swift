...
class ViewController: GLKViewController, ARSessionDelegate, RTABMapObserver, UIPickerViewDataSource, UIPickerViewDelegate, CLLocationManagerDelegate {
    ...
    private var reconstructionManager: ReconstructionManager?
    
    public var captureID: String = ""
    public var datasetID: Int = 1
    public var captureLimit: Int = 0 //0 = infinite, any other number will stop capturing at that limit.
    ...
    private func getSavingStateString(state: savingState) -> String {
        ...
        case .inititateResumeScanning:
            return "Inititated resume scanning"
        case .generatingMesh:
    ...
    @IBOutlet weak var ReconstructionSceneViewer: SCNView!
    ...
    */
    ...
    override func viewDidLoad() {
        ...
        networking = NetworkingManager.shared
        
        //Create timer
        timerManager = TimerManager()
        timerManager?.setInterval(duration: 3)
        
        reconstructionManager = ReconstructionManager()
        
        //Add callback for saving file
        timerManager?.callback = {
            if(self.captureLimit == 0 || self.datasetID < self.captureLimit)
            {
                self.enableScanningLoop = true
                self.saveIterationUninterrupted()
            }
            else if(self.datasetID >= self.captureLimit)
            {
                self.stopCapture()
            }
         }
    ...
    func startCamera()
    {
        ...
        let setIPAction = UIAlertAction(title: "Set", style: .default) { _ in
            if let textField = setIPPromptController.textFields?.first, let inputText = textField.text
            {
                ...
                self.networking?.setupNetworking()
                //self.networking?.startWebSocket()
            }
        }
    ...
    */
    ...
    func newScan() {
        ...
        */
        ...
        //}
        self.reconstructionManager?.initialize(reconstrID: self.captureID, viewerRef: self.ReconstructionSceneViewer)
    }
    ...
    func saveIterationUninterrupted() {
        ...
        self.copyAndUploadDatabase();
        //savingIteration = false;
        self.saveState = .standby;
    }
    
    func copyAndUploadDatabase() {
        ...
        DispatchQueue.global(qos: .background).async
        {
            ...
            //set state as saving DB file
            self.saveState = .inititateResumeScanning;
            
            //capture saved datasetID for displaying to user
            let olddataSetID = self.datasetID
            
            //update the datasetID to the next iteration dataset to be captured
            self.datasetID += 1;
            ...
            DispatchQueue.main.async
            {
                ...
                {
                    self.showToast(message: "Stopped scanning for session. Latest iteration: " + String(olddataSetID), seconds: 2.0)
                }
            }
    ...
    func startCapture()
    {
        ...
        if(reconstructionManager?.timerManager?.isEnabled == false)
        {
            reconstructionManager?.timerManager?.startTimer()
        }
    ...
    func displayModel() {
        ...
        ReconstructionSceneViewer.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([
            ReconstructionSceneViewer.heightAnchor.constraint(equalTo: view.heightAnchor, multiplier: 0.35), // 25% height of the screen
            ReconstructionSceneViewer.centerYAnchor.constraint(equalTo: view.centerYAnchor) // Center vertically in the view
        ])
    }
    ...
    func getReconstructedModel() {
        ...
        ReconstructionSceneViewer.allowsCameraControl = true
        //ModelViewer.backgroundColor = UIColor.black
        ReconstructionSceneViewer.cameraControlConfiguration.allowsTranslation = false
        ReconstructionSceneViewer.scene = modelViewerScene
    }
    */
    ...
    func setModelVisible(bool: Bool) {
        self.ReconstructionSceneViewer.isHidden = !bool;
