...
from serverGlobals import dotenv_config
from database.mongoDB.types import collections_dict, CollectionType
from utililites import modelIOTools

from database.mongoDB.documentSchemas import ModelData_Document, ModelObject_Document
from database.mongoDB import documentWriter
...
def _find_local_files():
    def get_model_versions(asset_id: str):
        versions_dir = asset_id + "/models"
        if not os.path.exists(versions_dir):
            return []
        return modelIOTools.get_path_directories(versions_dir)
    ...
    def check_files_exists(version_dir: str):
        ...
        return {
            ...
            "creation_date": modelIOTools.get_file_meta_data(version_dir + "/" + model_name)["created_date"]
        }
    global local_files
    local_files = {}
    asset_ids = modelIOTools.get_path_directories("captures")

    for id_folder in asset_ids:
...
def flush_and_regenerate_reconstructions():
    ...
    for key in local_files.keys():
        ...
        documentWriter.upload_unique_object(CollectionType.RECONSTRUCTIONS, data_doc, {"data_uid": key})
        ...
        for version in versions.keys():
            ...
            documentWriter.upload_unique_object(CollectionType.RECONSTRUCTIONS, obj_doc, {"data_ref_id": key, "version": version})