 ...
 * TODO: Overhaul in progress, script related to hierarchy not implemented yet
 * 
 */
...
import { ObjectHierarchyStateInfo } from "../utilities/data/ObjectsData";
import { EventData, EventListener } from "../utilities/delegates/EventListener";

export class UpdatedHierarchyEvent implements EventData{
  objectCount: number
...
export class ObjectHierarchy_Manager {
  private static instance: ObjectHierarchy_Manager;
  
  onUpdateHierarchyListener: EventListener<UpdatedHierarchyEvent>

  stateInfos: ObjectHierarchyStateInfo[];
  ...
  constructor() {
    ...
    this.onUpdateHierarchyListener = new EventListener<UpdatedHierarchyEvent>()

    ObjectHierarchy_Manager.instance = this;
  }

  public static get ActiveInstance(){
    return ObjectHierarchy_Manager.instance
  }
  ...
  _RecursiveAddChildCount(childCount: number, parentInfo: ObjectHierarchyStateInfo) {
  ...
  _RecursiveCreateStateInfo(
    ...
    const currStateInfo = new ObjectHierarchyStateInfo(modelState);
  ...
  _IsValidModelIndex(index: number) {
  ...
  ToggleModelVisibility(index: number) {
    if (!this._IsValidModelIndex(index)) {
  ...
  PickFromMenu(index: number) {
    ...
    if (modelState.mesh !== null) {
      ObjectPickingManager.ActiveInstance?.SelectObject(modelState.mesh);
    }
  }
