...
import { ObjectHierarchy_Manager, UpdatedHierarchyEvent } from "../ObjectHierarchy";
import { GuiMenuManager } from "../../gui/GuiMenu";
import { ObjectPickingManager, SelectMeshEvent } from "../../objectPickingSelection/ObjectPickingManager";


export class ObjectHierarchyMenuController {
  static instance: ObjectHierarchyMenuController;
  baseMenuElement: any;
  stackPanel: any;
  elements: any[];
  ...
  constructor() {
    ...
    ObjectHierarchyMenuController.instance = this;
  }
  ...
  Element_OnPointerDown(mousePos: any, eventState: any) {
    ...
    ObjectHierarchy_Manager.ActiveInstance?.PickFromMenu(eventState.target._idInArray);
    ObjectHierarchyMenuController.instance.lastPointerDownTime = performance.now();
  }
  ...
  Element_OnPointerUp(mousePos: any, eventState: any) {
    ...
    const objHierMenu = ObjectHierarchyMenuController.instance;
    ...
    if (control.name === "scrollViewer_window") {
      ObjectHierarchy_Manager.ActiveInstance?.SetStateInfoParent(
  ...
  ElementVisible_OnPointerDown(mousePos: any, eventState: any) {
    ...
    ObjectHierarchy_Manager.ActiveInstance?.ToggleModelVisibility(image.parent._idInArray);
    ObjectHierarchyMenuController.instance.RefreshAllElement();
  }
  ...
  RefreshElement(index: number) {
    const stateInfo = ObjectHierarchy_Manager.ActiveInstance?.stateInfos[index];
    ...
    const newImage = modelState.isVisible
      ? "editing/ui/Visible.png"
      : "editing/ui/NotVisible.png";
  ...
  ToggleModelVisibleState(index: number) {
    ...
    ObjectHierarchy_Manager.ActiveInstance?.ToggleModelVisibility(
   ...
   * @returns
   */
  PickAssignedMesh(evnt: SelectMeshEvent) {
    const _this = ObjectHierarchyMenuController.instance
    const index = ObjectHierarchy_Manager.ActiveInstance?.GetStateInfoIndex(
  ...
  DeleteSelectedModel() {
    ...
    ObjectHierarchy_Manager.ActiveInstance?.RequestDeleteModel(this.lastSelectedId);
  }

  _OnHierarchyUpdate(evnt: UpdatedHierarchyEvent){
    ObjectHierarchyMenuController.instance.SetVisibleElements(evnt.objectCount)
    ObjectHierarchyMenuController.instance.RefreshAllElement()
  }
  ...
  Init(advTexture: AdvancedDynamicTexture, menuManager: GuiMenuManager) {
    ...
    deleteButton.onPointerDownObservable.add(() => {
      ObjectHierarchyMenuController.instance.DeleteSelectedModel();
    });
    ...
    const objectHierarchy = ObjectHierarchy_Manager.ActiveInstance
    if(objectHierarchy){
